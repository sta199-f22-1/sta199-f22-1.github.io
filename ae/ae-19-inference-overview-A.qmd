...
The data provide convincing evidence that the proportion of steel roller coasters opened before 1970 is different than those opened after 1970.
...
-   **Question:** The slope you calculated and interpeted above is a sample statistic (an estimate).
    It is possible (and quite likely) that the true slope of the relationship between durations and top speeds of all roller coasters is not exactly equal to this value.
    How can we quantify the uncertainty around this estimate?
...
-   **Your turn:** Describe how we can construct a bootstrap distribution for the slope of the model predicting `top_speed` from `duration`.
    1.  Take a bootstrap sample of pairs of `top_speed` and `duration`.
    2.  Fit a model predicting `top_speed` from `duration` using the bootstrap sample and record the value of the slope.
